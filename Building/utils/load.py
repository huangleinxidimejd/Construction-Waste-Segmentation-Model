...
def DataAug(data, labels, size):
    ...
    #     ten_crop_labels.append(ten_crop(label[i], size))
...
def ten_crop(src, size, center_tf=None, center_bf=None, center_rf=None):
    ...
    * bottom-right crop (flipped horizontally)
    ...
    if h < oh or w < ow:
        raise ValueError(
            "Cannot crop area {} from images with size ({}, {})".format(str(size), h, w))
