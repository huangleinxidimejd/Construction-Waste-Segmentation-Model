...
def rotate_image(image, angle, order=0):
    """
    Rotates an OpenCV 2 / NumPy images about it's centre by the given angle
    (in degrees). The returned images will be large enough to hold the entire
    new images, with a black background
    """

    # Get the images size
    # No that's not an error - NumPy stores images matricies backwards
    ...
    # Obtain the rotated coordinates of the images corners
    rotated_coords = [
    ...
    # Find the size of the new images
    ...
    # We require a translation matrix to keep the images centred
    trans_mat = np.matrix([
...
def create_crops(ims, labels, size):
    ...
    for img, label in zip(ims, labels):
        ...
        c_w = size[1]
        if h < c_h or w < c_w:
            print("Cannot crop area {} from images with size ({}, {})".format(str(size), h, w))
...
def sliding_crop(img, size):
    ...
    c_w = size[1]
    if h < c_h or w < c_w:
        print("Cannot crop area {} from images with size ({}, {})"
              .format(str(size), h, w))
    else:
        h_rate = h/c_h
        w_rate = w/c_w
...
def crop_around_center(image, width, height):
    """
    Given a NumPy / OpenCV 2 images, crops it to the given width and height,
