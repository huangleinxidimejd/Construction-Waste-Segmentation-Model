...
class traditional_DeepLabv3_plus(nn.Module):
    def __init__(self, in_channels=3, num_classes=7, pretrained=True):
        super(traditional_DeepLabv3_plus, self).__init__()
    ...
    def forward(self, x):
        ...
        x1 = self.low(x1)
        x = torch.cat((F.interpolate(aux, x1.size()[2:], mode='bilinear'), x1), 1)  # 四倍上采样
        fuse = self.fuse(x)
